...
const navItems = [
  ...
  { name: 'Platform Guide', href: '/guide', icon: BookOpen },
...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 bg-slate-900 border-r border-slate-800/80 text-slate-300 min-h-screen sticky top-0 z-30">
      ...
      {/* Developed by Prakhar Mishra Tag */}
      <div className="mx-3 mt-3 px-3 py-1.5 bg-indigo-950/80 rounded-lg border border-indigo-800/60 flex items-center gap-2">
        <Code2 className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
        <span className="text-[11px] font-semibold bg-gradient-to-r from-indigo-300 via-violet-300 to-pink-300 bg-clip-text text-transparent">
          Developed by Prakhar Mishra
        </span>
      </div>

      {/* Mode Badge */}
      <div className="px-4 py-2.5 mx-3 mt-2 bg-slate-800/60 rounded-xl border border-slate-700/50 flex items-center justify-between">
      ...
      <div className="p-4 border-t border-slate-800/80 flex items-center justify-between bg-slate-950/40">
        <div className="flex items-center gap-3">
          ...
          <div className="truncate">
            <p className="text-xs font-semibold text-white truncate">Prakhar Mishra</p>
